...
use App\Models\Auditoria; // <-- Importamos el modelo
...
class AdminController
{
    ...
    private $auditoria; // <-- Propiedad para la auditoría
    ...
    public function __construct($db)
    {
        ...
        $this->auditoria = new Auditoria($db); // <-- Inicializamos la auditoría
...
{
    // Permitimos ID 1 (Super) y 2 (Sucursal)
...
{
    $this->verificarAdmin();
    // Solo el ID 1 (Tú) puede entrar aquí
 ...
 public function usuarios()
    {
        ...
        // También traemos los roles para el modal de edición
...
public function actualizarUsuarioAjax()
    {
        ...
        try {
            ...
            // Preparamos la consulta con los nuevos campos normalizados
            ...
            if ($res) {
                // Auditoría detallada: ahora guardamos el ID del rol asignado
    ...
    }public function guardarBanner()
{
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        try {
            ...
            // Validamos que sea realmente una imagen (Seguridad Pro)
            $check = getimagesize($_FILES['imagen']['tmp_name']);
            if ($check === false) {
                throw new Exception("formato_invalido");
            }

            // 2. LÓGICA DE ORDEN AUTOMÁTICO
            ...
            $nombre_archivo = time() . '_' . bin2hex(random_bytes(4)) . '.' . $extension; // Nombre único y seguro
            ...
            if (move_uploaded_file($_FILES['imagen']['tmp_name'], $ruta_destino)) {
                ...
                // ÉXITO: Redirección con mensaje positivo
            ...
            // ERROR: Redirección con detalle del error para el SweetAlert
    ...
    public function cargarProductosPorCodigosAjax()
    {
        ...
        try {
            ...
            // Convertimos el string "101,102" en un array [101, 102]
            ...
            // El primer parámetro es la sucursal, luego los códigos
           ...
           $resultado = array_map(function($prod) {
            ...
            return [
                ...
                'nombre'       => $prod['nombre'],
                // 🔥 CAMBIO CLAVE: Cambiamos 'stock' por 'stock_real'
    ...
    /**
     * Borra una marca destacada vía AJAX (eliminando también su imagen)
     */
    public function borrarMarcaAjax()
    {
        header('Content-Type: application/json');
        try {
            // Leemos el ID enviado desde el JS (marcas_mantenedor.php)
            $data = json_decode(file_get_contents('php://input'), true);
            $id = $data['id'] ?? 0;

            if ($id > 0) {
                // 1. Buscamos la ruta de la imagen en la BD para borrarla del servidor
                $stmt = $this->db->prepare("SELECT ruta_imagen FROM marcas_destacadas WHERE id = ?");
                $stmt->execute([$id]);
                $img = $stmt->fetchColumn();

                // 2. Si la imagen existe físicamente, la borramos
                if ($img && file_exists(__DIR__ . '/../../public/' . $img)) {
                    unlink(__DIR__ . '/../../public/' . $img);
                }

                // 3. Registramos la acción en la auditoría (Seguridad)
                if (property_exists($this, 'auditoria') && $this->auditoria) {
    ...
    public function borrarBanner($id)
    {
        ...
        // 🔥 AUDITORÍA: Borrar banner tradicional
    ...
    public function reordenarBannersAjax()
    {
        ...
        // 🔥 AUDITORÍA: Reordenamiento masivo
    ...
    public function buscarParaBannerAjax()
    {
        ...
        try {
            ...
            // 🔥 Usamos exactamente la misma lógica de "Gestión de Productos"
            // COALESCE(ps.stock, 0) nos trae el Stock Físico real de la sucursal
            $sql = "SELECT p.id, 
                           p.cod_producto, 
                           COALESCE(piw.nombre_web, p.nombre) as nombre,
                           (COALESCE(ps.stock, 0) - COALESCE(ps.stock_reservado, 0)) as stock,
            ...
            $resultado = array_map(function ($prod) {
                ...
                return [
                    ...
                    'cod_producto' => $prod['cod_producto'],
                    'nombre'       => $prod['nombre'],
                    // 🔥 CAMBIO CLAVE: Cambiamos 'stock' por 'stock_real' para que coincida con banner.js
                    'stock_real'   => (int)$prod['stock'],
                    'imagen'       => $urlImagen
                ];
            }, $productos);
    ...
    public function actualizarFechasAjax()
    {
        ...
        // 🔥 AUDITORÍA: Fechas de Banner actualizadas
        if ($res) {
    ...
    public function reordenarMarcasAjax()
{
    ...
    // Leemos el JSON que envía Sortable.js
    ...
    if (isset($data['ordenes']) && is_array($data['ordenes'])) {
        try {
            ...
            // Preparamos la consulta una sola vez para que sea súper rápido
...
public function buscarReemplazoAjax()
{
    ...
    try {
        ...
        // Buscamos el producto con su precio y stock (Misma lógica segura que usamos antes)
        $sql = "SELECT p.id, 
                       p.cod_producto, 
                       COALESCE(piw.nombre_web, p.nombre) as nombre,
                ...
                FROM productos p
                LEFT JOIN productos_info_web piw ON p.cod_producto = piw.cod_producto
                -- Unimos con la sucursal (aquí usamos la 29 por defecto, puedes ajustarlo)
                LEFT JOIN productos_sucursales ps ON p.cod_producto = ps.cod_producto AND ps.sucursal_id = 29
                WHERE (p.cod_producto LIKE :term1 OR p.nombre LIKE :term2 OR piw.nombre_web LIKE :term3)
                AND p.activo = 1
                LIMIT 15";
        ...
        $resultado = array_map(function($prod) {
            ...
            return [
                ...
                'stock'        => max(0, (int)$prod['stock_real']),
                'imagen'       => $urlImagen
            ];
        }, $productos);
    ...
    public function importarERP()
    {
        ...
        try {
            ...
            // 🔥 AUDITORÍA: Sincronización ERP Manual
